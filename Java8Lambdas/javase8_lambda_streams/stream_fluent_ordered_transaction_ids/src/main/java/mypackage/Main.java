...
public class Main {
	...
	public static void main(String[] args) {
		...
		List<Product> products = Arrays.asList(new Product(1, ProductType.BOOK, 80),
											  new Product(2, ProductType.ELECTRONICS, 180),
											  new Product(3, ProductType.FOOD, 20),
											  new Product(4, ProductType.BOOK, 50),
											  new Product(5, ProductType.ELECTRONICS, 120));
		
		System.out.println("---- Practice (Internal iteration) operation ----");
		List<Integer> ids = products.stream()
				.filter(p -> p.getProductType()==ProductType.ELECTRONICS)
				.sorted(Comparator.comparing(Product::getPrice).reversed())
				.map(Product::getProductId)
				.collect(Collectors.toList());
		
		ids.forEach(id -> System.out.println("product id = " + id));
